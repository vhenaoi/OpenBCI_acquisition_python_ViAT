...
class TimeFrequency(object):
    def __init__(self,id_Subject,cc_Subject,date,loc,save):
        ...
        self.__date = date
        self.loc = loc +'/'+ str(self.__subject)+'_'+str(self.__cc) + '/' + date
#        self.loc = loc
        self.path_save=save +'/'+ str(self.__subject)+'_'+str(self.__cc) + '/' + date
        
    def plot_stft(self):
        name = str(self.__subject)+'_'+str(self.__cc)
        signal = pd.read_csv(self.loc+'/'+'Record_' + name +'.csv',sep=';', index_col=0)
#        signal = pd.read_csv(self.loc +'/'+ name +'.csv', header=None)
        fs= 250
        
        f, t, Zxx = sig.stft( signal['C2'], fs, nperseg = 500)
        
        levels = MaxNLocator(nbins=15).tick_values(np.abs(abs(Zxx)).min(), np.abs(abs(Zxx)).max())
